...
static PASSWORD: LazyLock<String> = LazyLock::new(|| env::var("PASSWORD").unwrap_or_default());

static RE_HTTP: LazyLock<Regex> = LazyLock::new(|| regex::Regex::new(r"^http:\/{1,2}").unwrap());

static RE_HTTPS: LazyLock<Regex> = LazyLock::new(|| regex::Regex::new(r"^https:\/{1,2}").unwrap());
...
async fn handler(Path(path): Path<String>, request: Request) -> impl IntoResponse {
    ...
    let new_path = RE_HTTPS
        .replace(
            RE_HTTP.replace(new_path, "http://").to_string().as_str(),
            "https://",
        )
        .to_string();
    // Some ingress will combine // as /
    let has_protocol = new_path.starts_with("http:/") || new_path.starts_with("https:/");
    let new_path_protocol = if has_protocol {
        new_path
    } else {
        format!("https://{}", new_path)
    };
